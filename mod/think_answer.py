...
def main():
	# load text
	rampo_text = load_from_file(path+'dataset/【公式】かよちんの言葉.txt')
	...
	sentence = text_model.make_sentence_with_start('今朝')

	mecab = MeCab.Tagger('-Oyomi')
	print(' '.join(mecab.parse(sentence).split()))	# need to concatenate space-splitted text

	# save learned data
	with open(path+'dataset/learned_data.json', 'w') as f:
